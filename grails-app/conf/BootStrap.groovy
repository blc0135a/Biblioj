...
class BootStrap {
    ...
    def init = { servletContext ->
		if (!Livre.count()){
			...
			def dicker = new Auteur(nom:"Dicker", prenom:"Joël").save(failOnError: true)
			def jonasson = new Auteur(nom:"Jonasson", prenom:"Jonas").save(failOnError: true)
			
			new Livre(titre:"Rien ne s'oppose à la nuit : roman", auteurs:vigan, type:nouveaute).save(failOnError: true)
			new Livre(titre:"Hunger games [Texte imprimé]", auteurs:collins, type:ado).save(failOnError: true)
			...
			new Livre(titre:"1Q84. 3. Octobre-décembre", auteurs:murakami, type:nouveaute).save(failOnError: true)
			new Livre(titre:"La révolte", auteurs:collins, type:ado).save(failOnError: true)
			...
			new Livre(titre:"La reine dans le palais des courants d'air [Texte imprimé]", auteurs:larsson, type:adulte).save(failOnError: true)
			...
			new Livre(titre:"La vérité sur l'affaire Harry Quebert : roman", auteurs:dicker, type:adulte).save(failOnError: true)
			new Livre(titre:"Le vieux qui ne voulait pas fêter son anniversaire : roman", auteurs:jonasson, type:nouveaute).save(failOnError: true)
